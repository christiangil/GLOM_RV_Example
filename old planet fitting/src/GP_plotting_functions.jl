...
function GLOM_line_plots(
    ...
    n_samp_points::Integer = convert(Int64, max(500, round(2 * sqrt(2) * length(prob_def.GLO.x_obs))))
    ) where {T<:Real}


    x_samp = collect(range(minimum(prob_def.GLO.x_obs), stop=maximum(prob_def.GLO.x_obs), length=n_samp_points))
    n_total_samp_points = n_samp_points * prob_def.GLO.n_out
    n_meas = length(prob_def.GLO.x_obs)
    ...
    # if find_post
    show_resids = true
    mean_GP, σ, mean_GP_obs, Σ = GLOM.GP_posteriors(prob_def.GLO, x_samp, total_hyperparameters; return_mean_obs=true, y_obs=remove_kepler(prob_def, fit_ks))

    # init_plot()
    # plot(x_samp, mean_GP[1:n_samp_points])
    # plot(prob_def.GLO.x_obs, mean_GP_obs[1:n_meas])
    ...
    mean = add_kepler(mean_GP, x_samp .* prob_def.time_unit, fit_ks; data_unit=prob_def.rv_unit*prob_def.GLO.normals[1])
    # mean = copy(mean_GP)
    mean_obs = add_kepler(mean_GP_obs, prob_def.time, fit_ks; data_unit=prob_def.rv_unit*prob_def.GLO.normals[1])
    # mean_obs = copy(mean_GP_obs)

    for output in 1:prob_def.GLO.n_out

        if plot_Σ_profile
            init_plot()
            fig = plot(collect(1:(prob_def.GLO.n_out * n_samp_points)) / n_samp_points, Σ[convert(Int64, round((output - 1 / 2) * n_samp_points)),:])
        ...
        sample_output_indices = output:prob_def.GLO.n_out:n_total_samp_points
        obs_output_indices = output:prob_def.GLO.n_out:length(prob_def.GLO.y_obs)

        # getting the y values for the proper output
        y_o = prob_def.GLO.y_obs[obs_output_indices]
        obs_noise_o = prob_def.GLO.noise[obs_output_indices]
        ...
        if output==1
            y_o .*= prob_def.GLO.normals[output]
            obs_noise_o .*= prob_def.GLO.normals[output]
            show_curves_o .*= prob_def.GLO.normals[output]
            σ_o .*= prob_def.GLO.normals[output]
            mean_o .*= prob_def.GLO.normals[output]
            # if find_post
            mean_obs_o .*= prob_def.GLO.normals[output]
            # end
        end

        show_resids ? axs = custom_GP_plot(x_samp, show_curves_o, prob_def.GLO.x_obs, y_o, σ_o, mean_o; errors=obs_noise_o, mean_obs=mean_obs_o) : axs = custom_GP_plot(x_samp, show_curves_o, prob_def.GLO.x_obs, y_o, σ_o, mean_o; errors=obs_noise_o)
        ...
        logL = -GLOM.nlogL_GLOM(prob_def.GLO, total_hyperparameters; y_obs=remove_kepler(prob_def, fit_ks))
        ...
        axs[1].text(minimum(prob_def.GLO.x_obs), 0.9 * max_val, L"\ell_{act}(\theta|t,s): " * string(convert(Int64, round(logL))), fontsize=30)
        # end

        # put kernel lengths on plot
        # kernel_lengths = total_hyperparameters[end-prob_def.GLO.n_kern_hyper+1:end]
        ...
        axs[1].text(minimum(prob_def.GLO.x_obs), 1 * min_val, hyper_param_string, fontsize=30)
        # text(minimum(prob_def.GLO.x_obs), 1 *minimum(y_o), "Wavelengths: " * string(kernel_lengths), fontsize=30)
    ...
    if fit_ks.K != 0u"m/s"
        ...
        samp_x = collect(range(0, stop=ustrip(fit_ks.P), length=1000)) * unit(fit_ks.P)
        keps = fit_ks.(samp_x)
        # ys = ustrip.(prob_def.rv) - mean_GP_obs[1:prob_def.GLOn_out:end] .* prob_def.GLOnormals[1]
        ys = (prob_def.GLO.y_obs[1:prob_def.GLO.n_out:end] - mean_GP_obs[1:prob_def.GLO.n_out:end]) .* prob_def.GLO.normals[1]
        # noises = convert_and_strip_units.(unit(prob_def.rv[1]), prob_def.rv_noise)
        noises = prob_def.GLO.noise[1:prob_def.GLO.n_out:end] .* prob_def.GLO.normals[1]
