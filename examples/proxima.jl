...
# importing ESPRESSO data
star_str = "ProximaCen"
data_dir = "examples/"
data = CSV.read(data_dir * star_str * "_ESPRESSO.csv", DataFrame)
...
save_dir = data_dir * "data/proxima/$kernel_name/" * star_str * "_"
fig_dir = data_dir * "figs/proxima/$kernel_name/" * star_str * "_"

# CHANGE: rvs and their errors go here
# obs_rvs = data[!, "vrad"][sort_inds]
obs_rvs = data[!, "vradTM"][sort_inds]
...
# obs_rvs_err = data[!, "svrad"][sort_inds]
obs_rvs_err = data[!, "svradTM"][sort_inds]
...
# to fit
add_indicator!(rvs_and_inds_holder, rvs_and_inds_err_holder, data[!, "fwhm"][sort_inds], data[!, "sfwhm"][sort_inds])
...
# If all a's active:
# glo = GLOM.GLO(kernel_function, num_kernel_hyperparameters, n_dif, n_out, obs_xs, copy(obs_ys); noise=copy(obs_noise), a=[[0.3 2e-4 0];[0.75 -1e-4 0]])
